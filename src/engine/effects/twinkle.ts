...
export class TwinkleEffect implements AsciiEffect {
  ...
  update(_dt: number, time: number, _mask: MaskGrid): EffectCell[] {
    ...
    for (const s of this.stars) {
      ...
      cells.push({ row: s.r, col: s.c, char: ch, brightness: pulse });
      if (s.big && pulse > 0.6) {
        cells.push({ row: s.r, col: s.c - 1, char: ".", brightness: pulse * 0.5 });
        cells.push({ row: s.r, col: s.c + 1, char: ".", brightness: pulse * 0.5 });
        if (pulse > 0.9) {
          cells.push({ row: s.r - 1, col: s.c, char: ".", brightness: pulse * 0.3 });
        }
      }
    }
    return cells;
