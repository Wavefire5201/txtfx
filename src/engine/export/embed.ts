...
meteor:function(p){var ms=[],rate=p.spawnRate||0.3,sn=p.speedMin||15,sx=p.speedMax||30,tl=p.tailLength||8,acc=0;return function(dt,t,out){acc+=rate*dt;if(acc>=1){acc-=1;ms.push({c:~~(Math.random()*C),r:0,sp:sn+Math.random()*(sx-sn),tl:tl+~~(Math.random()*4)});}for(var i=ms.length-1;i>=0;i--){var m=ms[i];var ang=(p.angle||-75)*Math.PI/180;m.r-=Math.sin(ang)*m.sp*dt;m.c+=Math.cos(ang)*m.sp*dt;if(m.r-m.tl>R){ms.splice(i,1);continue}var hr=~~m.r,hc=~~m.c;for(var j=0;j<m.tl;j++){var r=hr-j,c=hc+~~(j*.3);if(r>=0&&r<R&&c>=0&&c<C)out.push({r:r,c:c,ch:j===0?"@":j<3?"#":j<5?"*":".",b:1-j/m.tl});}}};},
...
glitch:function(p){var bl=[],int=p.intensity||.3,bs=p.blockSize||6,acc=0,gc="!@#$%&*<>[]{}|/\\\\~";return function(dt,t,out){acc+=int*dt*3;if(acc>=1){acc-=1;var w=2+~~(Math.random()*bs),h=1+~~(Math.random()*(bs/2));bl.push({c:~~(Math.random()*Math.max(1,C-w)),r:~~(Math.random()*Math.max(1,R-h)),w:w,h:h,life:0,mx:.1+Math.random()*.3});}for(var i=bl.length-1;i>=0;i--){var b=bl[i];b.life+=dt;if(b.life>b.mx){bl.splice(i,1);continue}for(var r=0;r<b.h;r++)for(var c=0;c<b.w;c++){var rr=b.r+r,cc=b.c+c;if(rr>=0&&rr<R&&cc>=0&&cc<C)out.push({r:rr,c:cc,ch:gc[~~(Math.random()*gc.length)],b:.7+.3*Math.random()});}}};},
scanline:function(p){var sp=p.speed||8,w=p.width||3,br=p.brightness||1,cnt=p.count||2,chs=p.chars||"=-~";return function(dt,t,out){for(var s=0;s<cnt;s++){var phase=(s/cnt)*R;var head=((t*sp+phase)%(R+w))-w;for(var wi=0;wi<w;wi++){var r=~~(head+wi);if(r<0||r>=R)continue;var tt=wi/w,b=br*(1-tt*.6),ch=chs[Math.min(wi,chs.length-1)]||"=";for(var c=0;c<C;c++){var fl=Math.sin(c*.5+t*12+s*3)*.15;out.push({r:r,c:c,ch:ch,b:Math.max(.1,b+fl)});}}}};},
...
decode:function(p){var bt=null,sp=p.duration||2.4,ramp="@#W$9876543210?!abc;:+=-,._ ";return function(dt,t,out){if(!bt)bt=baseText.split("\\n");for(var r=0;r<Math.min(bt.length,R);r++){var ln=bt[r]||"";for(var c=0;c<ln.length;c++){if(ln[c]===" ")continue;var delay=((c/C+r/R)/2*(p.diagonalBias||0.7)+(1-(p.diagonalBias||0.7))*Math.random())*sp*2;var el=t-delay;if(el<0){if(Math.random()<.15)out.push({r:r,c:c,ch:ramp[~~(Math.random()*(ramp.length-1))],b:.3});}else if(el<(p.settleTime||0.4)){out.push({r:r,c:c,ch:Math.random()<.5?ln[c]:ramp[~~(Math.random()*(ramp.length-1))],b:.7});}else{out.push({r:r,c:c,ch:ln[c],b:1});}};};},
"custom-emitter":function(p){var parts=[],rate=p.rate||10,sp=p.speed||8,life=p.life||2,sprd=p.spread||6.28,acc=0,pch=p.chars||"*+.";return function(dt,t,out){acc+=rate*dt;var n=~~acc;acc-=n;var ox=C*(p.spawnX||0.5),oy=R*(p.spawnY||1.0);for(var i=0;i<n;i++){var dir=(p.direction||-90)*Math.PI/180;var a=dir+Math.random()*sprd-sprd/2;parts.push({x:ox,y:oy,vx:Math.cos(a)*sp*(.5+Math.random()),vy:Math.sin(a)*sp*.45*(.5+Math.random()),life:0,mx:life*(.5+Math.random())});}for(var i=parts.length-1;i>=0;i--){var pp=parts[i];pp.life+=dt;if(pp.life>pp.mx){parts.splice(i,1);continue}pp.x+=pp.vx*dt;pp.y+=pp.vy*dt;pp.vy+=(p.gravity||0)*dt;var r=~~pp.y,c=~~pp.x,tt=pp.life/pp.mx;if(r>=0&&r<R&&c>=0&&c<C)out.push({r:r,c:c,ch:pch[~~(tt*(pch.length-1))]||".",b:Math.pow(1-tt,2)});}};},
};
