...
declare const SCENE: {
  ...
  effects: Array<{
    ...
    timeline: { start: number; end: number | null; loop: boolean };
...
(function () {
  ...
  function animate() {
    ...
    function tick() {
      ...
      if (!loop && el > dur) return; // Stop non-looping
      const dt = Math.min(0.05, Math.abs(t - lastT));
      if (t < lastT - 0.1) initEffects(); // Loop wrap
      lastT = t;
      ...
      for (const cell of allCells) {
        ...
        if (b > brightMap[idx]) {
          brightMap[idx] = b;
          charMap[idx] = cell.char;
        }
      }
      let text = "";
      for (let r = 0; r < rows; r++) {
        if (r > 0) text += "\n";
        for (let c = 0; c < cols; c++) text += charMap[r * cols + c];
      }
      F.textContent = text;
          ...
          if (ctx) {
            ...
            for (const cell of allCells) {
              ...
              if (gr > 0) {
                ...
                grad.addColorStop(0, `rgba(${cR},${cG},${cB},${a * 0.7})`);
                grad.addColorStop(0.4, `rgba(${cR},${cG},${cB},${a * 0.28})`);
